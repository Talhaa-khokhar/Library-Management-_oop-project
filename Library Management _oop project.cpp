...
void login_screen()
{
    ...
    cout<<"\t\t\t\t";
    header_bar();
    cout<<"\n\n\t\t\t\t  (1)Teachers\t(2)Students\t(3)Staff"<<endl;
    cout<<"\t\t\t\t";
    header_bar();
...
void Staff_Screen_Menu(unsigned int an)
{
    cout<<"\n";
    header_bar();
    cout<<"\nAccount Number: "<<an;
    cout<<"\n(1)  Search for books"<<endl;
    cout<<"(2)  Issued Books List"<<endl;
    cout<<"(3)  Return Book"<<endl;
    cout<<"(4)  ReIssue Book"<<endl;
    cout<<"(5)  Issue Book"<<endl;
    cout<<"(6)  Remove Fine"<<endl;
    cout<<"(7)  Calculate Fine"<<endl;
    cout<<"(8)  Enter new book"<<endl;
    cout<<"(9)  Check User accounts"<<endl;
    cout<<"(10) Check own account"<<endl;
    cout<<"(11) Enter new Magazines/Journal/Newspapers"<<endl;
    cout<<"(12) Enter new Staff"<<endl;
    cout<<"(13) Enter new Student"<<endl;
    cout<<"(14) Enter new Teacher"<<endl;
    cout<<"(15) Show Categories"<<endl;
    header_bar();
    cout<<"\n";
}
...
public:
    ...
    Teacher(string n, unsigned int an, double f, int bi)
    {
        setName(n);
        setAccount_Number(an);
        setBooksIssue(bi);
    ...
    {
        header_custom("Input Teacher Info");
        cout<<"\nEnter Name: ";
        cin>>Name;
        cout<<"Enter Account Number: ";
        cin>>Account_Number;
    ...
    {
        header_custom("Display of Teacher's info");
        cout<<"Name: "<<getName()<<endl;
        cout<<"Account Number: "<<getAccount_Number()<<endl;
        cout<<"Total Fine: "<<getFine()<<endl;
        cout<<"Books issued: "<<getBooksIssue()<<endl;
...
class Library
{
    ...
    int BOOKS, STUDENTS, TEACHERS, STAFFS, NMJ;
    string Category;
    string Issue;
...
public:
    ...
    Library(Book b1, Book b2, Book b3, Book b4, Book b5, Student stu1, Student stu2, Student stu3, Teacher t1, Teacher t2, Staff s1, Newspaper_Magzines_Journals nmj)
    {
        books[0]=b1;
        books[1]=b2;
        books[2]=b3;
    ...
    void setTEACHERS(int t)                 {TEACHERS=t;}
    ...
    void Issue_Book_Teacher()
    {
        ...
        header_custom("Issue Book");
                ...
                for (int i=0 ; i<=4 ; i++)          //Since there are only 5 students
                {
                    if(getAccNum()==teachers[i].getAccount_Number())
                    {
                        Show_Category();
                        if (teachers[i].getBooksIssue()<=10)
                        {
                        Search_Book_Issue(teachers[i].getAccount_Number(),num);
                        teachers[i].setBooksIssue(teachers[i].getBooksIssue()+1); //This mean increment in their book
                        }
                        else
                            cout<<"Maximum Limit reached!"<<endl;
                    }
            ...
            case 2:
                {
                    for (int i=0 ; i<=9 ; i++)
                    {
                        if(books[i].getTitle()==getSearch())
                        {
                            for (int j=0 ; j<=4 ; j++)
                            {
                                if(getAccNum()==teachers[j].getAccount_Number())
                                {
                                    cout<<"Book returned!";
                                    books[i].setStatus("Available");
                                    books[i].setAccountNum(0);
                                    books[i].Return_Date.~Date();       //using destructor to set date back to 0.
                                    teachers[j].setBooksIssue(teachers[j].getBooksIssue()-1);
                                    check_one=true;
                                }
                            }
                            if(check_one==false)
            ...
            case 2:
                {
                    for (int i=0 ; i<=9 ; i++)
                    {
                        if(books[i].getTitle()==getSearch())
                        {
                            for (int j=0 ; j<=4 ; j++)
                            {
                                if(teachers[j].getAccount_Number()==getAccNum())
                                {
                                    books[i].Return_Date.increment_date(90);
                                    cout<<"Book reissued successfully!"<<endl;
                                    check_one=true;
                                }
                            }
            ...
            case 2:
                {
                    ...
                    for(int i=0 ; i<=9 ; i++)
                    {
                        if (books[i].getTitle()==getSearch())     //finding book in book collection
                        {
                            ...
                            for (int j=0 ; j<=4 ; j++)
                            {
                                if (teachers[j].getAccount_Number()==getAccNum())  //finding user in users
                                {
                                    teachers[j].Display_Teacher_info();
                                    books[i].Change_Return_Date();              //This will ask for the return date.
                                    Fine_days=books[i].Issued_Date.Total_Days(books[i].Return_Date);
                                    if( Fine_days>90)
                                        {
                                            teachers[j].setFine(Fine_days*10);
                                            cout<<"Total Fine: "<<teachers[j].getFine()<<endl;
                                        }
                                    else
                                        cout<<"No Fine"<<endl;
                                }
            ...
            case 2:
                {
                    for (int i=0 ; i<=4 ; i++)
                    {
                        if(teachers[i].getAccount_Number()==getAccNum())
                        {
                            teachers[i].setFine(0);
                            cout<<"Fine removed successfully!"<<endl;
                            check_one=true;
                        }
                    }
    ...
    void Enter_New_Teacher()
    {
        header_custom("Enter new Teachers");
        if(getTEACHERS()<=4)
        {
            teachers[getTEACHERS()].Input_Teacher_info();
            if (getTEACHERS()<4)
                setTEACHERS(getTEACHERS()+1);
        }
        else
            cout<<"Maximum capacity reached!"<<endl;
    }
    ...
    void Issued_Book_Of_Users(int n, unsigned int an)
    {
        switch (n)
        {
            case 1:
            {
                header_custom("Teacher's Issued Books");
                for (int i=0 ; i<=9 ; i++)
                {
                    for (int j=0; j<=4 ; j++)
                    {
                        if( (books[i].getStatus()=="Issued") && (teachers[j].getAccount_Number()==an)     )
                            books[i].Display_Book_Info();
                    }
                }
                break;
    ...
    void Display_User_info(int n, unsigned int an)
    {
        switch (n)
        {
            case 1:
            {
                for (int i=0 ; i<=4 ; i++)
                {
                    if(an==teachers[i].getAccount_Number())
                        teachers[i].Display_Teacher_info();
                }
            }
            case 2:
            {
...
int main()
{
    ...
    Book book_2 ("Discrete","Gilbert",4554464654668, 2005, 002, "Maths", "Available", 0, 5);
    Book book_3 ("Harry","JK_Rowling",79454645645682,1999,003,"English","Available",0,10);
    Book book_4 ("Maths","Gamer101",124454455,2019,004,"Computer_Science","Available",0,5);
    Book book_5 ("Seerat","Shibli_Naumani",74544515345611,1917,005,"Urdu","Available",0,5);

    Student stu_one ("Amna",120,500,0); //120 is the account number
    Student stu_two ("Fatima",121,0,0);
    Student stu_three ("Ali",122, 20,0);

    Teacher tea_one ("Usman", 5, 0,0); //05 is the account number
    Teacher tea_two ("RabiaBasri", 06, 0,0);

    Staff sta_one ("Salman",50,30,0);   //50 is the account number


    Library lib(book_1, book_2, book_3, book_4, book_5, stu_one, stu_two, stu_three, tea_one, tea_two, sta_one, nmj_one);
        ...
        case 3:
            {
                while (1)
                {
                    if (lib.Find_User(Account_Number,3) == true)
                    {
                        Staff_Screen_Menu(Account_Number);
                        cout<<"Press the option from above:"<<endl;
                        cin>>option_menu;
                        switch(option_menu)
                        {
                            ...
                            case 9:
                            {
                                lib.Display_All_Accounts();
                                break;
                            }
                            case 10:
                            {
                                lib.Display_User_info(3,Account_Number);
                                break;
                            }
                            case 11:
                            {
                            ...
                            case 13:
                            {
                                lib.Enter_New_Student();
                                break;
                            }
                            case 14:
                            {
                                lib.Enter_New_Teacher();
                                break;
                            }
                            case 15:
                            {
